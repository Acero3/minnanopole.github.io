  ...
  if (isset($_POST['btn_add_recoil'])){
    ...
    print_r ($quantity);
      for ($i=0; $i<count($quantity);$i++){
        if($quantity[$i] > 0){
        $Product->addCartRecoil($user_id, $product_id[$i], $quantity[$i]);
        echo $product_id[$i];
        echo $quantity[$i];
        }
      }
  ...
  <body>
    <form  method="post">
      <div class="container my-5 pt-5">
        <img class="img-fluid bg-black" src="../materials/chose_recoil.png" alt="">
        <div class="table-responsive mt-5">
          <table class="table table-sm">
            ...
            <tbody>
              <div class="row">
                <?php
                  $all_poles_recoil = $Product->getProductsRecoil();
                  while($pole_recoil = $all_poles_recoil->fetch_assoc() ){
                ?>
                <tr>
                  <td class="col-2">
                    <?=$pole_recoil['length']?>
                  ...
                  <td class="boader-bottom-1 col-4">
                    ...
                    <input 
                    <?php
                    if($pole_recoil['stocks'] <= 0){
                      echo "disabled ";
                    }
                    $max_qty_recoil = $pole_recoil['stocks'];
                    echo "max = '$max_qty_recoil'";
                    ?>
        ...
        <div class="row chose_btn">
          <button name="btn_add_recoil" class="btn btn-warning btn-lg mt-5 font-weight-bold mx-auto col-4" type="submit" href="#" role="button"><i class="fa-solid fa-cart-shopping"></i>   カートに入れる</button>
        </div>
      </div>
    </form>
